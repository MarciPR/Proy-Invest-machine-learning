...
model = tf.keras.models.load_model("ModeloMNISTentrenado.h5")
...
def ImagePreProcess(im_orig):
    ...
    #img_resized = resize(im_bw,(28,28), mode='constant')
    #io.imshow(img_resized) 
    #plt.show()
    #cv2.imshow("Window", img_resized)
    ##########################################################
    #invert image
    im_gray_invert = cv2.bitwise_not(img_resized)
    cv2.imshow("Window", im_gray_invert)
    ####################################
    im_final = im_gray_invert.reshape(1,28,28)
    #cv2.imshow("Window", im_final)
    ...
    print('El resultado predicho por la red es: ',ans)
